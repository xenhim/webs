...
function curl_get_contents( $url, $image = false){
	...
	if($scraperapi && !$image){
		$settings = FF_CRAWLER_HELPERS::get_crawler_settings();

        $scraper_service = isset($settings['proxy']['scraper_service']) ? $settings['proxy']['scraper_service'] : 'scrapperapi';

        switch($scraper_service){
            case 'scrapingbee':
