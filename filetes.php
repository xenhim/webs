<?php 
// Đọc nội dung file link.txt vào một mảng
$links = file('alllinks.txt', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);

// Lặp qua từng link trong mảng
foreach ($links as $url) {
    // Chạy mã PHP cho mỗi link
    processUrl($url);
}

function processUrl($url) {
    // Toàn bộ mã PHP của bạn ở đây
    // ...
    // Sử dụng biến $url để lấy dữ liệu và xử lý
    // ...
...
$ii = 1;
function uploadToCloudflare($imageUrl, $url, $chap2) {
	global $ii;
    $prefix = "https://www.nettruyenus.com/truyen-tranh/";
    $slug = str_replace($prefix, "", $url);
	$chap = str_replace("Chương ", "Chuong-", $chap2);
    //echo $slug."-".$chap."-".$ii."\n";
...
$imageUrl = $link_img;
//echo $imageUrl."\n";
$responseJson = uploadToCloudflare($imageUrl, $url, $chap2);
